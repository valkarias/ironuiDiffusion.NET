...
public sealed unsafe class DiffusionModel : IDisposable
{
	...
	public int vaeProcess(int key, bool decode) {
	...
	//public int Inpaint(string prompt, IImage image, IImage mask, DiffusionParameter? parameter = null)
	//{
		//parameter ??= GetDefaultParameter();
//
		//ObjectDisposedException.ThrowIf(_disposed, this);
...
//
	...
	public int ImageToImage(ExtraImage2ImageParameters inferenceParams, string prompt, DiffusionParameter parameter = null) {
		parameter ??= GetDefaultParameter();
		...
		int init_key = inferenceParams.initialKey;
		int image = inferenceParams.imageIndex;
		int mask = inferenceParams.maskIndex;

		int result = Constants.EMPTY_INDEX;
		if (parameter.ControlNet.IsEnabled)
		{
			if (parameter.ControlNet.Image is not IImage<ColorRGB> controlNetImage)
				controlNetImage = parameter.ControlNet.Image!.ConvertTo<ColorRGB>();
